...
export const ParentHomePage = () => {
  ...
  useEffect(() => {
    async function fetchFamilyMembersData() {
      if (formInput.profile.relation === "AYAH") {
        const { data } = await getFamilyMembersByHeadPhone(
          formInput.profile.phoneNumber
        );
        userStore.setState({ familyMembers: data });
        return;
      }
      const { data } = await getFamilyMembersByHeadPhone(
        fatherFormInput.profile.phoneNumber
      );
      userStore.setState({ familyMembers: data });
    }
  ...
  console.log({ familyMembers, formInput });
    ...
    <article className="w-full">
      <section>
        ...
        <BasicTable
          ...
          data={familyMembers?.length > 0 ? familyMembers : []}
          format={format}
