...
const ResidenceForm = ({ residenceStatus = "", address = "", description = "", onInputChange, isResidenceSame, children }) => {
    return (
        <div className="flex flex-col p-4 border rounded-xl mt-4 gap-4">
            ...
            <div className="flex flex-col gap-2">
                ...
                <Select onValueChange={status => onInputChange('status', status, 'residence')} value={isResidenceSame ? JSON.parse(localStorage.getItem('formInput')).residence.status : residenceStatus} disabled={isResidenceSame}>
            ...
            <FormInputText name={"address"} onChange={({ target }) => onInputChange(target.name, target.value, 'residence')} title={"Alamat"} value={isResidenceSame ? JSON.parse(localStorage.getItem('formInput')).residence.address : address} placeholder={"Masukkan alamat anda"} type='textarea' isDisabled={isResidenceSame} />
            {children}
        </div>
    )
}
